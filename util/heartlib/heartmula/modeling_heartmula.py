...
class HeartMuLa(PreTrainedModel):
    ...
    def generate_frame(
        ...
        # 1. Capture the target device (CPU or MPS)
        target_device = tokens.device

        # 2. Build the math tables and immediately move them to the target device
        for module in [self.backbone, self.decoder]:
            for layer in module.layers:
                if hasattr(layer.attn.pos_embeddings, 'rope_init'):
                    layer.attn.pos_embeddings.rope_init()
                    layer.attn.pos_embeddings.to(target_device) # Move table to match tokens
        ...
        for layer in self.backbone.layers:
            h = layer(h, mask=curr_backbone_mask, input_pos=input_pos)
        ...
        for i in range(1, self.config.audio_num_codebooks):
            ...
            # )
            # DECODER BYPASS
            h_dec = self.projection(curr_h)
            for layer in self.decoder.layers:
                h_dec = layer(h_dec, mask=curr_decoder_mask, input_pos=curr_pos)
            decoder_h = self.decoder.norm(h_dec)
